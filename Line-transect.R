...
#####change#####
#x<-overlay(new.grid,transect)#changed
...
#####end of changes#####
    ...
    d<-d[!is.na(d$square),]
          ...
          #if new.grid not clipped with sphm
            #d$square_area<-(size/1000)^2 #in kilometers2
    ...
    #analyse- calcul des densitées telles que demandées
    
#####change#####
#x<-distance.wrap(d,stratum=stratum,empty=empty,detection=detection,lsub=lsub,split=split,path=path,pathMCDS=pathMCDS,breaks=breaks,STR_LABEL=STR_LABEL,STR_AREA=STR_AREA,SMP_LABEL=SMP_LABEL,SMP_EFFORT=SMP_EFFORT,DISTANCE=DISTANCE,SIZE=SIZE,verbose=FALSE)
